...
using stop_req_pipe = ext::intel::pipe<class PipeStopRequest, int, 0>;

extern "C" {
  event pr_sink1(queue &q, buffer<long>& a_buf, buffer<size_t>& status_buf, size_t init_addr) {

    return q.submit([&](handler &h) {
      // Create an accessor with write permission.
      accessor a(a_buf, h, write_only, no_init);
      accessor status(status_buf, h, write_only, no_init);

      h.single_task<class pr_sink_test> ([=]() { 

        size_t addr = init_addr;
        ...
        while(true) {
          ...
          // flop stop_valid once it is 1
          if (!stop_valid) {
            read_stop =  stop_req_pipe::read(stop_valid);
          }
          read_data = read_iopipe::read(data_valid);


          // drain valid data before stopping
          if (data_valid) {
            // If read data is valid, write it to the buffer
            a[addr] = read_data;
            addr++;
          } 
          else if (stop_valid) {
            // If stop request is valid, break the loop
            status[0] = addr; // tell host which address to resume work
            break;
          }
  ...
  void stop_req_kernel1(queue &q) {

    q.submit([&](handler &h) {

      h.single_task<class stop_req> ([=]() { 

        stop_req_pipe::write(1);
      });
